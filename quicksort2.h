...
int * q_sort(double number[], int left, int right, int MAX){//pass the data array, the starting index and ending index of the data array, and the size of the array
...
//start_tick = clock();
           ...
           quickSort(number,left,right,MAX,index);
           /*for(i=0;i<MAX;i++)
           printf("%d ",number[i]);
           */
...
quickSort(double number[], int left, int right, int MAX,int index[]) {  
     if(left < right) {         
             int i = left;         
             int j = right + 1;  
             int m;       
