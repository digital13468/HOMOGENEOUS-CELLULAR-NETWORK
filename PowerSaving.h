...
heuristic(int Tbs, double *P, int Ntp, int MP, int DSt, double BP, int *DS, int *Xbs, int *Ybs, int *Xtp, int *Ytp, double *ptrD, double *BR){
	...
	double TotalBandwidth;
       ...
       SortedPowerRequirement=SortedPowerRequirementAssignment(P,Tbs,Ntp);
                         ...
                         /*for(i=0;i<Tbs;i++)
                         for(j=0;j<Ntp;j++)
                         printf("BS%d MS%d: %lf \n",i+1,j+1,*(SortedPowerRequirement+i*Ntp+j));*/
				...
				if(BStatus[*(SortedCumulativePowerIndex+k)]==0){	
					...
					if(NewTransmissionPower-ABP-SBP>MP){
						...
						if(RemovableUsers>0){
							...
							Index=q_sort(CombinedBSs,0,CombinedSize-1,CombinedSize);
							j=CombinedSize-1;
					...
					if(NewTransmissionPower-ABP-SBP<=MP){
						...
						if(SavablePower>MaxSavablePower&&*(CumulativeDS+*(SortedCumulativePowerIndex+i))+*(CumulativeDS+*(SortedCumulativePowerIndex+k))<=DSt){
							...
							if(RemovableUsers>0){
								...
								Index=q_sort(CombinedDSs,0,CombinedSize-1,CombinedSize);
								j=CombinedSize-1;
								if(CombinedSize>RemovableUsers)
									while(j>CombinedSize-RemovableUsers-1){
										XDS+=CombinedDSs[j];
