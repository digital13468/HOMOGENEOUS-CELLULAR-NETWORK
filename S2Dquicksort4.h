...
Sarrayprint(double *ptr, int BS, int MS){
       int i,j;         
       for(i=1;i<=BS;i++){
...
double * SSortedPowerRequirementAssignment(double *PowerRequirment,int Tbs,int Ntp){
       int i,j;
       ...
       *(SortedPower+i*Ntp+j)=*(PowerRequirment+(i+1)*(Ntp+1)+j+1);
       //printf("Unsorted Power Requirement from BS %d to MS %d: %lf \n",i+1,j+1,*(SortedPower+i*Ntp+j));
       }
       return SortedPower;                                
}         
                                                  
int * Sq_sort_2D(double *number, int low, int hight, int MS, int BS){//pass the data array, the starting index and ending index of the data array, and the size of the array
...
//start_tick = clock();
           ...
           *(index+j*MS+i)=j;
           ...
           //printf("index size %d\n",BS*(right-left+1));
           for(i=0;i<MS;i++){
           SquickSort2D(number,low,hight,MS,i,index,BS);}
           /*for(i=0;i<16;i++){
           printf("%d ",*(index+i));
           printf("%lf ",*(number+i));}*/
...
SquickSort2D(double *number, int low, int hight, int MS,int MSTerm, int *index, int BS) {  
     //printf("%d %d\n",low,hight);
     if(low < hight) {         
             int i = low;         
             int j = hight + 1;  
             ...
             //index[j]=left;
                             ...
                             printf("3. %d %d %d %d\n",low,hight,i,j );*/
             SquickSort2D(number, low, j-1, MS, MSTerm, index, BS);  //printf("former ends\n");  printf("later starts\n");        
             SquickSort2D(number, j+1, hight, MS, MSTerm, index, BS);/*printf("later ends\n");
